...
int main()
{
  ...
  WWDG_SetCounter(0); //置看们狗计数值(也可作为喂狗), 0~255, 最长约600ms溢出
  WWDG_ResetCfg(ENABLE); //开启看门复位
  /* 配置串口调试 */
  DebugInit();
  PRINT( "Start @ChipID=%02X\n", R8_CHIP_ID );
  
#if 1       /* 定时器0，设定100ms定时器进行IO口闪灯， PB10-LED */

  GPIOB_SetBits( GPIO_Pin_10 );
  GPIOB_ModeCfg( GPIO_Pin_10, GPIO_ModeOut_PP_5mA );

  TMR0_TimerInit( FREQ_SYS / 10 /1000);                  // 设置定时时间 100ms
...
/* 开启计数溢出中断，计慢1000个周期进入中断 */
  ...
  GPIOB_ModeCfg( GPIO_Pin_7, GPIO_ModeOut_PP_5mA ); //PB7
